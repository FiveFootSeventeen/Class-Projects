	 ...
	 float megaBodyPerSec; 
         ...
         for( int t = 0; t < NUMSTEPS; t++ )
         {
                 ...
                 for( int i = 0; i < NUMBODIES; i++ )
                 {
                          ...
                          float fz = 0.;
                          Body *bi = &Bodies[i];
			  #pragma omp parallel for default(none), reduction(+:fx,fy,fz),  shared(Bodies, i, bi), schedule(dynamic)
 	 ...
 	 megaBodyPerSec = ((float)(NUMBODIES*NUMBODIES*NUMSTEPS)/(time1-time0)/100000.);
	 printf("MegaBodies Compared Per Second: %2.2lf\n", megaBodyPerSec);
