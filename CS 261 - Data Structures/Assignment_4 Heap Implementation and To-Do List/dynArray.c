/*********************************************************************
** Program Filename: dynArray.c
** Author: Jacob Leno
** Date: 03/01/2018
** Description: Function implementations for the heap data structure.
*********************************************************************/
...
{
  	TYPE temp;
	temp = heap->data[0];	//Get the first(smallest) value in the heap
  	return temp;
}
...
void addHeap(DynArr *heap, TYPE node)
{
	assert(heap != 0);
	double parent;
	double pos = sizeDynArr(heap);
	addDynArr(heap, node);

	while (pos != 0) {	
		parent = (int)((pos - 1) / 2);
		if (compare(getDynArr(heap, pos), getDynArr(heap, parent)) == -1) {
			swapDynArr(heap, parent, pos);		//Check the current node with the value of its parent
			pos = parent;					//if it is smaller than its parent percolate it up.
		}
		else return;
	}
}
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	if (rightChild < max) {		//If rightChild is less than max there are two children
		if (compare(getDynArr(heap, leftChild), getDynArr(heap, rightChild)) == - 1)	
	...
	else if (leftChild < max) {	//There is only one child, we need only to check the left child
...
{
	int lastElement = sizeDynArr(heap) - 1;
	assert(sizeDynArr(heap) > 0);		//Make sure there is at least one element
	putDynArr(heap, 0, getDynArr(heap, lastElement));
	removeAtDynArr(heap, lastElement);	//Remove the last element and adjust
	_adjustHeap(heap, lastElement, 0);
}
...
void _buildHeap(DynArr *heap)
{
	...
	int i;
	for (i = (max / 2) - 1; i >= 0; i--) {		//Start from first non-leaf node and 
		_adjustHeap(heap, max, i);				//call adjust heap on each node prior to that
	}
...
{
	int i;						//Start by swapping the first(smallest) value with the last value and iterate over the entire heap
	_buildHeap(heap);			//after each iteration swap the next value the the first (which will always be the next smallest) 
