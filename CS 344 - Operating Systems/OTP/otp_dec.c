 ...
 *Date: 06/09/18
 *Program: otp_dec
 *Description: otp_dec is given a ciphertext file as its first argument
 * and a key file as it second argument. Its third argument is
 * the port at which to connect to otp_dec_d. otp_dec will
 * send these two files to otp_dec_d and wait to receive back
...
{	
	...
	int socketFD, portNumber, charsRead, charsWritten = -5, processType;
	int continueRead = 1, bufferLen, msgLen;
	struct sockaddr_in serverAddress;
	struct hostent* serverHostInfo;
	char buffer[50], *decBuffer, *keyBuffer, *encBuffer;

	encBuffer = malloc(DA_BUFFER_SIZE * sizeof(char)); //Alloc at least one byte so that strlen doesnt segfault
    	keyBuffer = malloc(DA_BUFFER_SIZE * sizeof(char));
	memset(encBuffer, '\0', DA_BUFFER_SIZE); //Set the byte to the null terminator
	memset(keyBuffer, '\0', DA_BUFFER_SIZE);

	if (argc < 4) { fprintf(stderr,"USAGE: %s hostname port\n", argv[0]); exit(1); } // Check usage & args
	FILE *encFile = fopen(argv[1], "r"); //Open up the plaintext file for reading
		if(encFile == NULL) { fprintf(stderr, "failed to open file in argv[1]\n"); exit(1); }
	...
	while(continueRead){
		...
		if( fgets(buffer, sizeof(buffer), encFile) != NULL) bufferLen = strlen(buffer); //Get info in the encrypted file and read it into the buffer
		...
		encBuffer = realloc(encBuffer, strlen(encBuffer) + bufferLen + 1); //Reallocate memory for the buffer
		strcat(encBuffer, buffer); //Concatenate the buffer onto the end of keyBuffer
	}
	encBuffer[strlen(encBuffer) - 1] = '\0'; //Remove the newline
	...
	if(strlen(encBuffer) > strlen(keyBuffer)) { fprintf(stderr, "Key file cannot be shorter than ciphertext file!\n"); exit(1); }
	if(checkBadChar(encBuffer, (size_t)strlen(encBuffer))) { fprintf(stderr, "Ciphertext file contains charcters other than A-Z or space!\n"); exit(1); }
	...
	if(ntohl(processType) != -40){ //Check to see if we have the correct process trying to connect
		fprintf(stderr, "Cannot use otp_enc_d unless connecting from otp_enc!\n");
		exit(2);
	}
	
	
	msgLen = htonl(strlen(encBuffer)); 
	write(socketFD, &msgLen, sizeof(msgLen)); //Send the length of the upcoming message before transfering the message
	
	if(writen(socketFD, encBuffer, strlen(encBuffer)) != strlen(encBuffer)){
	...
	decBuffer = malloc(strlen(encBuffer) * sizeof(char)); //Create space to store the dencrypted message
	memset(decBuffer, '\0', strlen(encBuffer)); 
	readn(socketFD, decBuffer, strlen(encBuffer)); //Read the dencrypted message into the buffer

	printf("%s\n", decBuffer); //Output the buffer to stdout with a newline
...
int checkBadChar(char *buffer, size_t length){
	...
	for(i = 0; i < length; i++){
		if((buffer[i] >  90 || buffer[i] < 65) && buffer[i] != 32) { //If a bad character is found return 0
			return 1;
		}
	}
	return 0;
