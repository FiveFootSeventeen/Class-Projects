 ...
 * 5 otp_dec proccesses at once. When otp_dec_d connects to a process
 * it will receive data from otp_dec and dencrypt it sending
 * the dendrypted message to otp_dec
 ***************************************************/
...
int main(int argc, char *argv[])
{
	...
	int spawnPid, i, msgLen, encLength, keyInt, encInt, otp_encID = -40;
	pid_t childPid;
	socklen_t sizeOfClientInfo;
	char *decBuffer, *keyBuffer, *encBuffer, temp;
	...
	childProcesses = 0;
	while(1){
		if(childProcesses >= 5){ //Spawn up to 5 child processes
			...
			switch(spawnPid){
				...
				case 0: { 
					...
					encLength = msgLen = ntohl(msgLen); //Return the value in msgLen back to host byte order
					
					encBuffer = malloc((msgLen * sizeof(char)) + 1); //Create enough memory to hold the entire message
					memset(encBuffer, '\0', msgLen + 1);
					if(readn(establishedConnectionFD, encBuffer, msgLen) != msgLen){
					...
					decBuffer = malloc((encLength * sizeof(char)) + 1); //Allocate enough space in the encBuffer to hold the encrypted message
					memset(decBuffer, '\0', encLength + 1); 
					for(i = 0; i < encLength; i++){
						encInt = getInt(encBuffer[i]); //Get the int associated with the current char in the ptBuffer
						keyInt = getInt(keyBuffer[i]); //Get the int associated with the current char in the keyBuffer
						temp = encInt - keyInt;
						if(temp < 0) temp += 27;
						decBuffer[i] = getChar(temp); //Get the encrypted char and store it in the encBuffer
					}
					
					writen(establishedConnectionFD, decBuffer, encLength);
